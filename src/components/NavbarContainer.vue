...
<template>
  ...
  >
    ...
    <div :class="isOpen ? 'flex absolute w-full bg-white shadow p-2' : 'hidden'" class="flex items-start flex-col md:flex md:flex-row gap-2 uppercase md:justify-between md:items-center ">
      <ul
        
        class="md:flex flex-col md:flex-row md:gap-2 md:w-1/2"
      >
        <li class="mr-6 pb-2 md:py-0" v-for="(item, index) in navs" :key="index">
          <a
            :href="item.link"
            :class="
